import { URL_ASSET_TAG } from '../../src/models/config.js';
...
describe('replaceAssetUrl', () => {
    ...
    test('with url with spaces', () => {
        expect(replaceAssetUrl(baseConfig, `${URL_ASSET_TAG}(the images/dummy image.jpg)`)).toEqual('assets-output/the images/dummy image.jpg');
    });
    test.skip('with url with parenthesis', () => {
        expect(replaceAssetUrl(baseConfig, `${URL_ASSET_TAG}(img/dummy(image).jpg)`)).toEqual('assets-output/img/dummy(image).jpg');
    });
});
//# sourceMappingURL=replaceAssetUrl.test.js.map