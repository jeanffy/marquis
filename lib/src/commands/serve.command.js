...
export async function serveCommand(args) {
    ...
    switch (args[0]) {
        case 'up':
            command = `docker compose -f "${pathes.srcRuntimeDockerCompose}" --project-directory "${pathes.rootDir}" up -d`;
            break;
        case 'down':
            command = `docker compose -f "${pathes.srcRuntimeDockerCompose}" --project-directory "${pathes.rootDir}" down`;
            break;
        default:
